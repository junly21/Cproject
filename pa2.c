...
static int process_instruction(unsigned int instr)
{
	...
	if (opcode == 0) {
		...
		int rt = (instr >> 16) % (1 << 5);
		int rd = (instr >> 11) % (1 << 5);
		int shamt = (instr >> 6) % (1 << 5);
		int funct = instr % (1 << 6);

		int reg_rs = registers[rs];
		int reg_rt = registers[rt];
		int reg_rd = registers[rd];
		//R format
		if (funct == 0x20) {//add
			registers[rd] = reg_rs + reg_rt;
		}
			...
			if (reg_rt >= 0)
				...
				registers[rd] = (registers[rt] >> shamt) + (0xffffffff - ((1 << shamt) - 1));
		}
		else if (funct == 0x2A) {//slt
			registers[rd] = (reg_rs < reg_rt) ? 1 : 0;
		}
		...
		if (opcode == 0x08) {//addi
			if (imm / (1 << 15) == 1)
				registers[rt] = (reg_rs + imm) % (1 << 16);
			else
				registers[rt] = reg_rs + imm;
		}
			...
			registers[rt] = (memory[loc] << 24) + (memory[loc + 1] << 16) + (memory[loc + 2] << 8) + (memory[loc + 3]);
		}
		else if (opcode == 0x2B) {//sw
			int loc = reg_rs + imm;

			int instr_1 = (reg_rt >> 24) % (1 << 8);
			int instr_2 = (reg_rt >> 16) % (1 << 8);
			int instr_3 = (reg_rt >> 8) % (1 << 8);
			int instr_4 = reg_rt % (1 << 8);

			memory[loc] = instr_1;
			memory[loc + 1] = instr_2;
			memory[loc + 2] = instr_3;
			memory[loc + 3] = instr_4;

		}
		else if (opcode == 0x0A) {//slti
			registers[rt] = (reg_rs < imm) ? 1 : 0;
		}
...
static int load_program(char* const filename)
{
	...
	while (fgets(command, sizeof(command), input)) {
		...
		int instr = strtol(tokens, NULL, 16);
		int instr_1 = (instr >> 24) % (1 << 8);
		int instr_2 = (instr >> 16) % (1 << 8);
		int instr_3 = (instr >> 8) % (1 << 8);
		int instr_4 = instr % (1 << 8);

		memory[INITIAL_PC + i] = instr_1;
		memory[INITIAL_PC + i + 1] = instr_2;
		memory[INITIAL_PC + i + 2] = instr_3;
		memory[INITIAL_PC + i + 3] = instr_4;

		i = i + 4;
	}
	memory[INITIAL_PC + i] = 0xff;
	memory[INITIAL_PC + i + 1] = 0xff;
	memory[INITIAL_PC + i + 2] = 0xff;
	memory[INITIAL_PC + i + 3] = 0xff;
...
static int run_program(void)
{
	...
	while (1) {
		...
		int check = process_instruction(instr);
