...
### ***Due on 00:00, November 13 (Saturday)***
...
- The machine has 32 registers and their values are in `unsigned int registers[32]`. Their name is specified in `const char *register_names[32]`, and their status can be examined using `show` CLI command. Without specifying the register name, `show` shows all register values as follows;
  ...
  You may specify register name to see the value of the register only. Note that register #0 is named to `zr`.
...
- The machine is equipped with 1MB memory which is accessible through  `unsigned char memory[]`. Like the registers, the framework provides `dump` CLI command to examine memory contents. It accepts the starting address and length for command parameters. Non-printable characters are printed as '.'.
  ```
  >> dump 0 32
...
- The emulator should support following instructions. Note that some conditional branch instructions(slt, slti, beq, bne), jump instructions(jr, j, jal), and a special instruction(halt) are added from PA1.
...
-  Let's extend this emulator to understand and process a **program**, *the list of machine instructions*. The program file contains a list of MIPS instructions, one instruction per line. So, the program file looks like;
...
- `load_program()` is to load a program into the memory. The function is invoked when you enter `load` command on the prompt with a file name to load. The program described in the specified file should be loaded from `INITIAL_PC`, and ends with the `halt` instruction. Assume a file "myprogram" contains following instruction;
...
- You can submit your code for testing up to 30 times.
- Some testcases will show only the decision (i.e., pass or fail) not the details. However, the their results will be used for grading.
